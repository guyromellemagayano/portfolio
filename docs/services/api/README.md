...
`apps/api` is the monorepo integration boundary used by public app clients such as `web` to consume portfolio-facing backend services through a stable API surface.
...
See `docs/standards/api/API_PORTFOLIO_STANDARDS.md` for folder conventions, response contracts, and integration checklist.
