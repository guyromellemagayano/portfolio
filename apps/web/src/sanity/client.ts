 ...
 * @description Lightweight Sanity query client using the Sanity HTTP query API.
 */
...
/** Reads a Sanity environment variable from the current runtime. */
function getSanityEnvVar(key: string): string {
  return globalThis.process.env[key]?.trim() ?? "";
}

/** Returns the current Sanity client configuration when required env vars exist. */
...
/** Returns `true` when the Sanity client has the minimum configuration required to query content. */
export function hasSanityConfig(): boolean {
  return getSanityConfig() !== null;
}

/** Executes a published-content query against the Sanity HTTP API. */
