...
it("renders synchronously with standard props", async () => {
  render(<Canvas data-testid="canvas-element">Synchronous canvas</Canvas>);

  // Renders the canvas immediately
  const canvas = screen.getByTestId("canvas-element");
  expect(canvas.tagName).toBe("CANVAS");
  expect(canvas).toHaveTextContent("Synchronous canvas");
...
it("renders repeated synchronous usage with standard props", async () => {
  render(<Canvas data-testid="canvas-element">Repeated canvas</Canvas>);

  // Renders the canvas immediately
  const canvas = screen.getByTestId("canvas-element");
  expect(canvas.tagName).toBe("CANVAS");
  expect(canvas).toHaveTextContent("Repeated canvas");
...
it("maintains state during synchronous rendering", () => {
  ...
  expect(canvas).toHaveAttribute("height", "600");
  expect(canvas).toHaveAttribute("aria-busy", "true");
  expect(canvas).toHaveTextContent("Rendered Canvas");
});
...
it("handles suspense error boundaries", () => {
  ...
  expect(() => {
    render(<Canvas data-testid="canvas-element">Error Boundary Test</Canvas>);
  }).not.toThrow();
});
