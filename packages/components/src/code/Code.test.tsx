...
it("renders synchronous render with standard props", async () => {
  render(<Code data-testid="code-element">client code</Code>);

  // Should render element immediately
  const code = screen.getByTestId("code-element");
  expect(code.tagName).toBe("CODE");
...
it("renders synchronous rendering with standard props", async () => {
  render(<Code data-testid="code-element">standard code</Code>);

  const code = screen.getByTestId("code-element");
  expect(code.tagName).toBe("CODE");
  expect(code).toHaveTextContent("standard code");

  await screen.findByTestId("code-element");
});

// ref forwarding test (only attaches with standard props in current implementation)
it("forwards ref correctly when rendered on client", () => {
  const ref = React.createRef<HTMLElement>();
  render(<Code ref={ref}>ref content</Code>);
