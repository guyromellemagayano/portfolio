...
it("renders synchronously with standard props", async () => {
  render(<Button data-testid="button-element">Synchronous button</Button>);

  const button = screen.getByTestId("button-element");
  expect(button.tagName).toBe("BUTTON");
  expect(button).toHaveTextContent("Synchronous button");
  expect(button).not.toBeDisabled();
  await screen.findByTestId("button-element");
});

it("renders repeated synchronous usage with standard props", async () => {
  render(<Button data-testid="button-element">Repeated button</Button>);

  const button = screen.getByTestId("button-element");
  expect(button.tagName).toBe("BUTTON");
  expect(button).toHaveTextContent("Repeated button");
  expect(button).not.toBeDisabled();
  await screen.findByTestId("button-element");
});
...
it("maintains state during synchronous rendering", () => {
  ...
  expect(button).toBeDisabled();
  expect(button).toHaveAttribute("aria-busy", "true");
  expect(button).toHaveTextContent("Rendered Button");
});
...
it("handles suspense error boundaries", () => {
  ...
  expect(() => {
    render(<Button data-testid="button-element">Error Boundary Test</Button>);
  }).not.toThrow();
});
