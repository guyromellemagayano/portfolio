...
it("renders synchronously with standard props", async () => {
  render(<B data-testid="bold-element">Synchronous bold text</B>);

  // Renders the bold immediately
  const bold = screen.getByTestId("bold-element");
  expect(bold.tagName).toBe("B");
  expect(bold).toHaveTextContent("Synchronous bold text");
...
it("renders repeated synchronous usage with standard props", async () => {
  render(<B data-testid="bold-element">Repeated bold text</B>);

  // Renders the bold immediately
  const bold = screen.getByTestId("bold-element");
  expect(bold.tagName).toBe("B");
  expect(bold).toHaveTextContent("Repeated bold text");
