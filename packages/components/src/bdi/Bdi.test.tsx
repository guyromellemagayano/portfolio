...
it("renders synchronously with standard props", async () => {
  render(<Bdi data-testid="bdi-element">Synchronous bidirectional</Bdi>);

  // Renders the bdi immediately
  const bdi = screen.getByTestId("bdi-element");
  expect(bdi.tagName).toBe("BDI");
  expect(bdi).toHaveTextContent("Synchronous bidirectional");
...
it("renders repeated synchronous usage with standard props", async () => {
  render(<Bdi data-testid="bdi-element">Repeated bidirectional</Bdi>);

  // Renders the bdi immediately
  const bdi = screen.getByTestId("bdi-element");
  expect(bdi.tagName).toBe("BDI");
  expect(bdi).toHaveTextContent("Repeated bidirectional");
