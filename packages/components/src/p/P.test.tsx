...
it("renders with standard props", async () => {
  render(<P data-testid="p-element">Synchronous content</P>);
  const el = screen.getByTestId("p-element");
  expect(el.tagName).toBe("P");
...
it("renders repeated synchronous usage with standard props", async () => {
  render(<P data-testid="p-element">Repeated content</P>);
  const el = screen.getByTestId("p-element");
  expect(el.tagName).toBe("P");
  expect(el).toHaveTextContent("Repeated content");
  await screen.findByTestId("p-element");
});

it("forwards ref when rendered", () => {
  const ref = React.createRef<HTMLParagraphElement>();
  render(<P ref={ref}>Ref</P>);
