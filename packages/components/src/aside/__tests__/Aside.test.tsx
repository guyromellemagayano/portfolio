...
it("renders synchronously with standard props", async () => {
  render(<Aside data-testid="aside-element">Synchronous sidebar</Aside>);

  // Renders the aside immediately
  const aside = screen.getByTestId("aside-element");
  expect(aside.tagName).toBe("ASIDE");
  expect(aside).toHaveTextContent("Synchronous sidebar");
...
it("renders repeated synchronous usage with standard props", async () => {
  render(<Aside data-testid="aside-element">Repeated sidebar</Aside>);

  // Renders the aside immediately
  const aside = screen.getByTestId("aside-element");
  expect(aside.tagName).toBe("ASIDE");
  expect(aside).toHaveTextContent("Repeated sidebar");
