...
// standard rendering props
it("renders synchronously with standard props", async () => {
  render(<Caption data-testid="caption-element">Synchronous caption</Caption>);

  // Renders the caption immediately
  const caption = screen.getByTestId("caption-element");
  expect(caption.tagName).toBe("CAPTION");
  expect(caption).toHaveTextContent("Synchronous caption");

  // The element remains queryable after render
  await screen.findByTestId("caption-element");
});

it("renders repeated synchronous usage with standard props", async () => {
  render(<Caption data-testid="caption-element">Repeated caption</Caption>);

  // Renders the caption immediately
  const caption = screen.getByTestId("caption-element");
  expect(caption.tagName).toBe("CAPTION");
  expect(caption).toHaveTextContent("Repeated caption");
...
it("maintains state during synchronous rendering", () => {
  render(
    <Caption
      data-testid="caption-element"
      className="table-caption"
  ...
  expect(caption).toHaveClass("table-caption");
  expect(caption).toHaveAttribute("aria-busy", "true");
  expect(caption).toHaveTextContent("Rendered Caption");
});

// Synchronous Render Tests
it("renders content synchronously", () => {
  render(
    <Caption data-testid="caption-element" className="table-caption">
      Loading Caption
    </Caption>
...
it("handles suspense error boundaries", () => {
  ...
  expect(() => {
    render(
      <Caption data-testid="caption-element">Error Boundary Test</Caption>
    );
  }).not.toThrow();
});
