...
it("handles CommonComponentProps correctly", () => {
  const { container } = render(
    <Body as="div" data-testid="test-body" className="test-class">
      Test content
    </Body>
  );
...
// Test prop - should render synchronously with standard props
it("renders synchronously with standard props", async () => {
  const { container } = render(
    <Body as="div" data-testid="test-body">
      Synchronous body content
    </Body>
  );

  // Renders the div immediately
  const divElement = container.querySelector('[data-testid="test-body"]');
  expect(divElement).toBeInTheDocument();
  expect(divElement?.tagName).toBe("DIV");
  expect(divElement).toHaveTextContent("Synchronous body content");
...
it("renders repeated synchronous usage with standard props", async () => {
  ...
  expect(divElement).toBeInTheDocument();
  expect(divElement?.tagName).toBe("DIV");
  expect(divElement).toHaveTextContent("Repeated body content");
